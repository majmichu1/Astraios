...
CATALOG_CACHE_DIR = Path.home() / ".astraios" / "catalogs"
...
def _query_vizier_tap(
    ...
    for attempt in range(3):
        try:
            req = urllib.request.Request(url, headers={"User-Agent": "Astraios/1.0"})
...
def plate_solve_astrometry_net(
    ...
    def _get_json(url: str) -> dict:
        return _request_with_retry(url, headers={"User-Agent": "Astraios/1.0"})
