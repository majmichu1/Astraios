"""Plugin system base classes for Astraios."""
...
PLUGIN_DIRS = [
    Path.home() / ".astraios" / "plugins",
    Path(__file__).parent / "builtin",
]


class AstraiosProcess:
...
class AstraiosPlugin:
    ...
    author: str = ""
    description: str = ""
    processes: list[type[AstraiosProcess]] = []
...
registry: dict[str, AstraiosProcess] = {}


def register_process(proc: AstraiosProcess):
    registry[proc.identifier] = proc
    log.info("Registered process: %s (%s)", proc.identifier, proc.name)


def get_process(identifier: str) -> AstraiosProcess | None:
    ...
    try:
        ...
        for attr_name in dir(mod):
            ...
            if (
                ...
                and issubclass(attr, AstraiosPlugin)
                and attr is not AstraiosPlugin
            ):
                plugin = attr()
                plugin.register()
